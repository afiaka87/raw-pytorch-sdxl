...
- **Quantization** (`--quantize int8` or `--quantize 4bit`): Reduces model weight memory by ~50% (uses PyTorch INT8)
...
- `--quantize`: Quantize UNet weights (int8 or 4bit, reduces model memory ~50%, uses PyTorch INT8)
