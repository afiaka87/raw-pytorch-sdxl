"""
SDXL UNet Implementation in Pure PyTorch

Adapted from minSDXL by Simo Ryu (https://github.com/cloneofsimo/minSDXL)
which is based on Hugging Face Diffusers (Apache 2.0 License)

This is a minimal, hackable implementation compatible with SDXL weights.
"""
...
class Upsample2D(nn.Module):
    """Upsample by factor of 2 using nearest neighbor interpolation + conv."""
...
class UNet2DConditionModel(nn.Module):
    """
    SDXL UNet with text conditioning.

    Architecture:
    - Input: 4-channel latents (128x128)
    - Encoder: 3 levels with [320, 640, 1280] channels
    - Middle: 1280 channels with attention
    ...
    def from_pretrained(cls, pretrained_path, torch_dtype=None, device=None):
        ...
        if not weights_path.exists():
            raise FileNotFoundError(f"UNet weights not found at {weights_path}")

        print(f"Loading UNet weights from {weights_path}...")
        ...
        # Do dtype conversion first if needed
        if torch_dtype is not None:
            model = model.to(dtype=torch_dtype)
            print(f"Converted UNet to {torch_dtype}")

        # Then move to device
        if device is not None:
            model = model.to(device=device)
            print(f"Moved UNet to {device}")
    ...
    def forward(
        ...
        Args:
            ...
            timesteps: [B] or scalar timesteps
            encoder_hidden_states: [B, seq_len, 2048] text embeddings
            added_cond_kwargs: dict with 'text_embeds' and 'time_ids'

        Returns:
            [sample]: List containing [B, 4, H, W] noise prediction
        """
        # Timestep embedding
        ...
        # Additional conditioning (SDXL-specific)
        ...
        sample, [s4, s5, s6] = self.down_blocks[1](
            sample, temb=emb, encoder_hidden_states=encoder_hidden_states
        )
